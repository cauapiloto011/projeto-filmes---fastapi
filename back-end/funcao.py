...
def inserir_filme(titilo, genero, ano, avaliacao):
    conexao, cursor = conectar()
    if conexao:
        try:
            cursor.execute(
                "INSERT INTO filmes (titilo, genero, ano, avaliacao) VALUES (%s, %s, %s, %s)",
                (titilo, genero, ano, avaliacao)
            )
            conexao.commit()
        except Exception as erro:
